...
class Restaurants(RestaurantsTemplate):
  ...
  def the_bearded_baristaa_button_click(self, x, y, button, **event_args):
    # Open The Bearded Baristaa's menu page.
    user = anvil.users.get_user()
    if user is not None:
      open_form('Menu')
    else:
  ...
  def restaurant_closed(self, x, y, button, **event_args):
    # Alert the user that the restaurant is closed.
    alert("Sorry, this restaurant is currently closed. The Bearded Baristaa is the only available restaurant at the moment.")